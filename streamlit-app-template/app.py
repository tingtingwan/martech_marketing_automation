...
st.set_page_config(
    page_title="Campaign Workflow Template",
...
DEFAULT_BRIEF = {
    ...
    "budget": 0,
    "timeline": "-",
    "brief": "Define objectives, KPIs, and constraints."
}
...
# Utilities to keep UI components unchanged when assets are missing
def _blank_image(width: int = 1, height: int = 1) -> Image.Image:
    try:
        from PIL import Image as PILImage  # type: ignore
        return PILImage.new("RGB", (max(1, width), max(1, height)), (255, 255, 255))
    except Exception:
        # As a last resort, return page icon if it's an image; else do nothing
        return None  # type: ignore


def _image_or_blank(path: Optional[str]) -> Optional[Image.Image]:
    try:
        if isinstance(path, str) and os.path.exists(path):
            return Image.open(path)
    except Exception:
        pass
    return _blank_image(10, 10)
...
# ============================================================
# MAIN APP (UI preserved; data providers return empty)
# ============================================================
...
def main():
    ...
    # Placeholder campaigns (empty)
    PRODUCTION_CAMPAIGNS: Dict[str, Dict[str, Any]] = {}
    BRIEFS_DATA = []
    ...
    if not st.session_state.get("workflow_started", False):
        ...
        if campaign_keys_top:
            ...
            st.selectbox("Choose a campaign", options=["No campaigns available"], index=0, key="selected_campaign_key", disabled=True)
            st.session_state["campaign_meta"] = {"brief_id": None, "brief_title": "-", "campaign_name": "-"}
            st.session_state.campaign_brief = DEFAULT_BRIEF.copy()
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        st.divider()
        st.button("🚀 Start Campaign Creation", use_container_width=True, key="start_disabled", disabled=not bool(PRODUCTION_CAMPAIGNS))
        if bool(PRODUCTION_CAMPAIGNS):
            if st.session_state.get("start_disabled"):
                st.session_state.campaign_id = st.session_state["campaign_meta"].get("brief_id")
                st.session_state["prod_generated"] = False
                st.session_state.workflow_state = {
                    "step": 0,
                    "agent": "idle",
        ...
        # Started workflow UI (with empty data providers)
        step = st.session_state.workflow_state.get("step", 0)
        pct = int((max(0, min(step, 5)) / 5) * 100)
        st.markdown("#### Overall Progress")
        st.progress(pct)
        ...
        st.markdown(
            ...
            f"<div class='brief-item'><div class='brief-item-label'>Campaign</div><div class='brief-item-value'>{html.escape(meta.get('campaign_name','-') or '-')}</div></div>"
            f"<div class='brief-item'><div class='brief-item-label'>Type</div><div class='brief-item-value'>{html.escape(brief.get('type','-') or '-')}</div></div>"
            f"<div class='brief-item'><div class='brief-item-label'>Lifecycle</div><div class='brief-item-value'>-</div></div>"
            f"<div class='brief-item'><div class='brief-item-label'>Medical Constraints</div><div class='brief-item-value'>-</div></div>"
            f"<div class='brief-item'><div class='brief-item-label'>Legal Requirements</div><div class='brief-item-value'>-</div></div>"
            "</div>",
            unsafe_allow_html=True
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with tabs[0]:
            st.subheader("📝 Briefing")
            st.selectbox("Select campaign", options=["No campaigns available"], index=0, key="brief_selected_campaign", disabled=True)
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.markdown("##### Campaign Summary")
            st.markdown(
                "<div class='brief-grid' style='margin-top:8px;'>"
            ...
            st.markdown('</div>', unsafe_allow_html=True)
            st.info("Next: open the “🎨 Production” tab to generate variations (requires your implementation).")
        ...
        with tabs[1]:
            ...
            with col_seed:
                st.markdown("#### Agent Steps")
                if not st.session_state.get("prod_prompt_generated", False):
                    st.button("🧠 Generate Expert Prompt", key="btn_gen_prompt", use_container_width=True, disabled=True)
                else:
                    st.success("Expert prompt generated")
                    _ep = st.session_state.get("prod_prompt_text") or ""
                    st.markdown(
                ...
                st.divider()
                if st.session_state.get("prod_prompt_generated", False) and not st.session_state.get("prod_seed_found", False):
                    st.button("🔎 Find Seed Image", key="btn_find_seed", use_container_width=True, disabled=True)
                elif st.session_state.get("prod_seed_found", False):
                    st.success("Seed image ready")
                    st.markdown("<div class='section-label'>Seed Image</div>", unsafe_allow_html=True)
                    st.markdown("<div class='img-card'>", unsafe_allow_html=True)
                    st.image(_image_or_blank(seed_image_path), use_column_width=True)
                    st.markdown(
                        f"<div class='img-meta'>Dimensions: <span class='img-sub'>-</span></div>",
                        unsafe_allow_html=True
                    )
                    st.markdown("</div>", unsafe_allow_html=True)
            ...
            with col_gen:
                ...
                if not st.session_state.get("prod_generated", False):
                    ...
                    st.markdown("<div class='section-label'>Generated Creative</div>", unsafe_allow_html=True)
                    st.markdown('<div class="img-card">', unsafe_allow_html=True)
                    st.image(_image_or_blank(_gen_path), use_column_width=True)
                    ts = st.session_state.get("prod_generated_ts") or "-"
                    model = st.session_state.get("prod_generated_model") or "-"
        ...
        with tabs[2]:
            ...
            if not brief_id:
                ...
                with st.spinner("🔎 Fetching compliance decision (placeholder)..."):
                    compliance = get_compliance_from_uc_timeout(brief_id, timeout=8)
                if not compliance:
                    st.warning(f"❌ No compliance record for {brief_id}")
                else:
